...
@dp.message_handler(lambda message: message.text == "Відгукнутися")
    ...
    async with state.proxy() as data:
        ...
        if data['current_question_index'] < len(options[data['current_option']]):
            ...
            await message.answer(
                f"Дякуємо за твій відгук! Зовсім скоро ми вийдемо до тебе на зв'язок 😉\nДо зустрічі 👋")
            await state.finish()
...
async def send_answers_to_admin(telegram_id):
    ...
    await bot.send_message(admin_chat_id, users[str(telegram_id)]["reg"] + "\n" + "\n".join(
        [f"{key}: {value}" for key, value in users[str(telegram_id)].items() if key not in ['reg']]))
    try:
        ...
        if "reg" in users[str(message.chat.id)] and "resume" in users[str(message.chat.id)]:
            await message.answer(message_text,
                                 reply_markup=types.ReplyKeyboardMarkup(resize_keyboard=True).add(
                                     types.KeyboardButton("Профіль"),
                                     types.KeyboardButton("Відгукнутися"),
                                     types.KeyboardButton("Переглянути активні вакансії")
                                 ))
            return

    await message.answer(
        "Привіт, ти звернувся/звернулась в компанію компанія \n"
        ...
        "Дякуємо!\n"
        "імя, Рекрутер компанія", reply_markup=types.ReplyKeyboardMarkup(resize_keyboard=True).add(
            types.KeyboardButton("Профіль")))
...
async def get_info(message):
    ...
    if "reg" in users[str(message.chat.id)] and "resume" in users[str(message.chat.id)]:
        await bot.send_message(message.chat.id, users[str(message.chat.id)]["reg"] + "\n" + "\n".join(
            [f"{key}: {value}" for key, value in users[str(message.chat.id)].items() if key not in ['reg']]),
                               reply_markup=types.ReplyKeyboardMarkup(resize_keyboard=True).add(
                                   types.KeyboardButton("Редагувати профіль"),
                                   types.KeyboardButton("Головна")))
